...
public class H19 : MonoBehaviour
{
	...
	*/
	...
	{
		FilePicker.Instance.onCompleteCallback += SendTextFileSelect;
		FilePicker.Instance.ShowPicker(true);
	}

	void SendTextFileSelect(string filePath)
	{
		FilePicker.Instance.onCompleteCallback -= SendTextFileSelect;
	...
	IEnumerator SendFileCoroutine(string filePath, bool addCr = true)
	{
		string[] textArray = System.IO.File.ReadAllLines(filePath);
	...
	void SaveLog()
	{
		...
		FilePicker.Instance.ShowPicker(true);
		//string filePath = System.IO.Path.Combine(Application.persistentDataPath, "H19LOG.TXT");
		//OnSaveLogSuccess(filePath);
	}
	...
	{
		H19.Instance.ResetToPowerUpConfig();
		H19.Instance.EraseScreen();
	}
}
