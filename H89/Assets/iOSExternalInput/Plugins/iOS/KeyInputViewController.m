...
{
    NSUInteger n = [presses count];
    for (int i = 0; i < n; i++)
    {
        UIPress *obj = [[presses allObjects] objectAtIndex:i];
        UIKey *key = obj.key;
        UIKeyboardHIDUsage keyCode = key.keyCode;

        NSMutableString *inputCharacters = [[NSMutableString alloc] init];
        [inputCharacters appendFormat:@"%ld", (long)keyCode];

        UnitySendMessage("ExternalInputController", "DidReceiveKeyDown", [inputCharacters UTF8String]);
    }
}
...
-(void)pressesEnded:(NSSet<UIPress *> *)presses withEvent:(UIPressesEvent *)event
{
    ...
    for (int i = 0; i < n; i++)
    {
        ...
        UnitySendMessage("ExternalInputController", "DidReceiveKeyUp", [inputCharacters UTF8String]);
    }
}
@end
